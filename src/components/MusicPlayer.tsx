...
const MusicPlayer: React.FC = () => {
  ...
  if (!hasStarted) {
    return (
      ...
      >
        <div
          className="animate-fade-in max-w-md w-full p-8 rounded-3xl shadow-xl relative overflow-hidden"
          style={{ backgroundColor: '#ffffff', border: '4px solid #ffffff', colorScheme: 'light' }}
        >
          ...
          <div className="relative z-10">
            ...
            <p
              ...
              style={{ color: '#6b7280' }}
            >
              Prepare-se para uma dose de fofura!<br/>Deseja ativar a música?
            </p>
            ...
            <div className="flex flex-col gap-3">
              <button
                ...
                disabled={isLoading}
                className="w-full font-bold py-4 rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
                style={{ backgroundColor: '#8BBF9F', color: '#ffffff' }}
              >
                {isLoading ? (
                  <>
              ...
              <button
                ...
                style={{ backgroundColor: '#ffffff', color: '#9ca3af', border: '1px solid #e5e7eb' }}
              >
                Entrar sem som
              </button>
            </div>
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex items-center gap-2 animate-fade-in">
      <div className={`transition-all duration-300 ${isPlaying ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10 pointer-events-none'}`}>
        <button
          onClick={toggleMute}
          className="p-3 rounded-full shadow-md border border-white transition-colors"
          style={{ backgroundColor: 'rgba(255,255,255,0.8)', color: '#8C6E5D' }}
      ...
      <button
        ...
        style={isPlaying
          ? { backgroundColor: '#F2D06B', color: '#4A7c59' }
        ...
        {isPlaying ? (
          <>
            <div className="flex gap-1 h-3 items-end">
              <div className="w-1 h-full animate-[bounce_1s_infinite]" style={{ backgroundColor: '#4A7c59' }}></div>
              <div className="w-1 h-2/3 animate-[bounce_1.2s_infinite]" style={{ backgroundColor: '#4A7c59' }}></div>
              <div className="w-1 h-full animate-[bounce_0.8s_infinite]" style={{ backgroundColor: '#4A7c59' }}></div>
            </div>
