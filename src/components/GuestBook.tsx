...
const GuestBook: React.FC = () => {
  ...
  const handleGenerateMessage = async (tone: 'funny' | 'emotional' | 'short', senderName: string) => {
    if (!senderName.trim()) { alert("Por favor, digite seu nome primeiro!"); return; }
    ...
    try {
      ...
      setTimeout(() => { setMessageText(fallback); setAiError(true); }, 500);
    } finally {
  ...
  const handleAdminToggle = () => {
    ...
    if (password === "Fr@ncisco2026!") { setIsAdmin(true); }
  ...
  return (
    <section id="mensagens" className="py-16 bg-safari-cream relative">
      <style>{`
        ...
        .animate-border-pulse { animation: border-pulse-yellow 3s ease-in-out infinite; }
      `}</style>
      <div className="container mx-auto px-4">
        <Reveal className="text-center mb-12">
          <h2 className="font-display text-4xl font-bold text-safari-dark mb-4">Mural de Recados</h2>
          <p className="text-gray-600 max-w-2xl mx-auto">Deixe um recadinho especial para o Francisco ler quando crescer.</p>
        </Reveal>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
                  <p className="text-xs text-blue-600 font-bold mb-3 flex items-center gap-1">
                    <Sparkles size={14} /> Sem criatividade? A IA ajuda:
                  </p>
                  <div className="flex gap-2 flex-wrap mb-2">
                    <button type="button" onClick={() => handleGenerateMessage('emotional', name)} disabled={isGenerating}
                    ...
                    <button type="button" onClick={() => handleGenerateMessage('funny', name)} disabled={isGenerating}
                      className="text-xs px-3 py-2 bg-white text-blue-600 border border-blue-200 rounded-lg hover:bg-blue-600 hover:text-white transition-colors disabled:opacity-50">
                      {isGenerating ? '...' : '😂 Engraçado'}
                    </button>
                    <button type="button" onClick={() => handleGenerateMessage('short', name)} disabled={isGenerating}
                      className="text-xs px-3 py-2 bg-white text-blue-600 border border-blue-200 rounded-lg hover:bg-blue-600 hover:text-white transition-colors disabled:opacity-50">
                      {isGenerating ? '...' : '⚡ Curto'}
                    </button>
                  </div>
                  {aiError && <p className="text-[10px] text-gray-400 italic">*Sugestão gerada automaticamente (IA offline).</p>}
                </div>
                <button type="submit"
                ...
                <div key={msg.id} className="relative group">
                  <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 relative z-10 hover:shadow-md transition-shadow animate-border-pulse">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="text-safari-yellow/30"><MessageSquareHeart size={24} /></div>
                      </div>
                    </div>
                    <p className="text-gray-600 italic text-lg leading-relaxed pl-2 border-l-2 border-safari-green/30">"{msg.message}"</p>
                  ...
                  {replyingTo === msg.id && (
                    <div className="mt-[-10px] ml-6 pt-6 pl-6 relative">
                      ...
                      <div className="bg-white p-4 rounded-xl border border-safari-yellow shadow-md relative z-20">
                        <p className="text-xs font-bold text-safari-dark mb-2">{msg.reply ? 'Editando resposta para' : 'Respondendo a'} {msg.author}:</p>
                        <textarea value={replyText} onChange={(e) => setReplyText(e.target.value)}
                          className="w-full p-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:border-safari-yellow bg-gray-50" rows={2}
                          placeholder="Escreva uma resposta carinhosa..."></textarea>
                        <div className="flex justify-end gap-2 mt-2">
                          <button onClick={() => { setReplyingTo(null); setReplyText(''); }} className="text-xs text-gray-500 hover:text-gray-700 px-3 py-1">Cancelar</button>
                          <button onClick={() => handleSubmitReply(msg.id)} className="text-xs bg-safari-dark text-white px-3 py-1 rounded-lg hover:bg-safari-green transition-colors font-bold">
                  ...
                  {msg.reply && replyingTo !== msg.id && (
                    <div className="mt-[-10px] ml-6 relative pt-6 pl-6 group/reply">
                      ...
                      <div className={`p-5 rounded-xl relative shadow-sm hover:shadow-md transition-all border ${isAdmin ? 'bg-orange-50 border-orange-200' : 'bg-safari-cream/80 border-safari-yellow/20'}`}>
                        <div className={`flex items-center gap-2 mb-2 pb-2 border-b justify-between ${isAdmin ? 'border-orange-200' : 'border-safari-brown/10'}`}>
                          <div className="flex items-center gap-2">
                            <div className={`p-1 rounded-full shadow-sm ${isAdmin ? 'bg-orange-100' : 'bg-white'}`}>
                              {isAdmin ? <Crown size={12} className="text-orange-500 fill-orange-500" /> : <Heart size={12} className="text-red-400 fill-red-400" />}
                            </div>
        ...
        <div className="flex flex-col items-center mt-12 gap-2 opacity-40 hover:opacity-100 transition-opacity">
          ...
          {isAdmin && (
            <span className="text-[10px] text-orange-500 flex items-center gap-1 bg-orange-50 px-2 py-1 rounded">
              <AlertCircle size={10} /> Modo Administrador Ativo
            </span>
          )}
        </div>
      </div>
